...
App.searchResultsController=Em.ArrayController.create({
	...
	refresh:function(){
		...
		var query=self.get("query");
			var url="http://search.twitter.com/search.json?q="+query+App.currentOption.value+"&callback=?"
...
App.selectedOptionController=Em.ArrayController.create({
	content:[
		Em.Object.create({name: 'normal', id:""}),
		Em.Object.create({name:'positive', id:":)"}),
		Em.Object.create({name:'negative', id:":("})
	]
});

App.currentOption=Em.Object.create({
	valueOption:"",
	valueBinding:Em.Binding.oneWay('valueOption.id')
})
